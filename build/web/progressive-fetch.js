...
// polled as the fetch completes. It also can handle compression
async function progressiveFetch(resource, callbacks={}, { handleCompression=false }={}) {
    ...
    function update() {
        ...
        const loadedPercent = (loaded * 100.0).toFixed(2);
    ...
    if (handleCompression) {
        const compressionStream = new DecompressionStream('gzip');
        return new Response(response.body.pipeThrough(transform).pipeThrough(compressionStream), {
            headers: {
